...
> ```mermaid
> graph LR
>    GPIO[pin_button] --> R[Резистор 10К Ом]
>    R --> GND
>    BUTTON[Кнопка] --> GPIO
>    BUTTON --> VCC[3.3V]
> ```
...
> ```mermaid
> graph LR
>    VCC[3.3V] --> BUTTON[Кнопка]
>    BUTTON --> GPIO[pin_button]
>    GPIO -->|Внутренняя подтяжка| GND
> ```

> ### `PULLDOWN`
> ```mermaid
> graph LR
>    GPIO[pin_button] -->|Внутренняя подтяжка| GND
>    BUTTON[Кнопка] --> GPIO
>    BUTTON --> VCC[3.3V]
> ```
...
> 
> graph LR
>    GPIO[pin_led] --> LED[Светодиод] --> R[Резистор 220 Ом] --> GND
> ```
